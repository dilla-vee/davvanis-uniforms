...
function Avatar({ imageUrl, name, large }) {
  ...
  return (
    <div className={`${size} rounded-full bg-indigo-100 flex items-center justify-center font-bold text-indigo-600`}>
      {initials}
    </div>
  );
}
    ...
    <div className="space-y-5">
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {[1,2,3,4,5,6].map(i => <div key={i} className="card p-5 h-36 animate-pulse bg-gray-100" />)}
        </div>
      ) : clients.length === 0 ? (
            ...
            >
              <div className="flex items-start gap-3 mb-3">
                <div className="relative shrink-0">
                  ...
                  {client.image_url && (
                    <div className="w-12 h-12 rounded-full bg-indigo-100 items-center justify-center font-bold text-indigo-600 text-base absolute top-0 left-0 hidden">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-indigo-600 font-medium truncate">{client.school}</p>
                  {client.contact && <p className="text-xs text-gray-500 mt-0.5">{client.contact}</p>}
                </div>
              </div>
              {client.description && (
                <p className="text-xs text-gray-500 line-clamp-2 mb-3">{client.description}</p>
              )}
              <div className="flex gap-2 pt-2 border-t border-gray-50" onClick={e => e.stopPropagation()}>
                <button onClick={(e) => openEdit(e, client)} className="text-indigo-600 hover:text-indigo-800 text-xs font-medium">Edit</button>
                <button onClick={(e) => handleDelete(e, client.id)} className="text-red-500 hover:text-red-700 text-xs font-medium ml-auto">Delete</button>
              </div>
            </div>
          ))}
        </div>
          ...
          {detailLoading ? (
            ...
            <div className="space-y-5">
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h2 className="text-xl font-bold text-gray-900">{clientDetail.name}</h2>
                  <p className="text-indigo-600 font-medium">{clientDetail.school}</p>
                  {clientDetail.contact && <p className="text-sm text-gray-500 mt-1">📞 {clientDetail.contact}</p>}
                  {clientDetail.email && <p className="text-sm text-gray-500">✉️ {clientDetail.email}</p>}
                </div>
              </div>
              ...
              {clientDetail.description && (
                <div className="bg-gray-50 rounded-lg p-3">
                  ...
                  <p className="text-sm text-gray-700">{clientDetail.description}</p>
                </div>
              )}
                ...
                {clientDetail.orders?.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {clientDetail.orders?.map(order => (
                      <div key={order.id} className="border border-gray-100 rounded-lg p-3 flex items-center justify-between">
                        <div>
                          <span className="text-xs font-medium text-indigo-600">Order #{order.id}</span>
                          <span className="text-xs text-gray-400 mx-2">—</span>
                          <span className="text-xs text-gray-500">{new Date(order.order_date).toLocaleDateString()}</span>
                          <div className="mt-0.5 text-xs text-gray-500">{order.item_count} item{order.item_count !== 1 ? 's' : ''}</div>
                        </div>
                        <div className="text-right">
                          <div className="font-semibold text-gray-900 text-sm">£{(order.total_price || 0).toFixed(2)}</div>
        ...
        <Modal title={editClient ? 'Edit Client' : 'Add Client'} onClose={() => setShowForm(false)}>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex items-center gap-4">
              <div className="w-16 h-16 rounded-full overflow-hidden bg-indigo-50 flex items-center justify-center shrink-0">
                {imagePreview ? (
              ...
              <div>
                ...
                <input
                  ...
                  className="text-sm text-gray-500 file:mr-2 file:py-1 file:px-3 file:rounded file:border-0 file:text-xs file:font-medium file:bg-indigo-50 file:text-indigo-600 hover:file:bg-indigo-100"
                />
              </div>
            </div>
