...
function ChartTooltip({ active, payload, label, prefix='' }) {
  ...
  return (
    <div className="card px-3 py-2 text-sm shadow-lg" style={{borderRadius:'0.5rem'}}>
      ...
      <p className="text-theme-secondary">{prefix}{prefix==='ÂKsh '?Number(payload[0].value).toFixed(2):payload[0].value}</p>
    </div>
  );
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        <StatCard icon="ðŸ“¦" label="Unique Items"      value={summary?.total_items??0}                          bg="rgba(99,102,241,0.1)" />
        <StatCard icon="ðŸ”¢" label="Total Units"       value={summary?.total_units??0}                          bg="rgba(59,130,246,0.1)" />
        <StatCard icon="ðŸ’·" label="Inventory Value"   value={`ÂKsh ${(summary?.total_value??0).toFixed(2)}`}       bg="rgba(34,197,94,0.1)"  />
        <StatCard icon="âš ï¸" label="Low Stock Items"   value={summary?.low_stock_count??0}                      bg="rgba(239,68,68,0.1)"  />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="card p-5">
          <h3 className="text-base font-semibold text-theme-primary mb-4">Inventory Value by Category (ÂKsh )</h3>
          {categories.length===0 ? <p className="text-center py-10 text-theme-muted text-sm">No data available.</p> : (
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={categories} margin={{top:5,right:10,left:0,bottom:5}}>
                <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" />
                <XAxis dataKey="name" tick={{fontSize:12,fill:'var(--text-secondary)'}} />
                <YAxis tick={{fontSize:12,fill:'var(--text-secondary)'}} tickFormatter={v=>`ÂKsh ${v}`} />
                <Tooltip content={<ChartTooltip prefix="ÂKsh " />} />
              ...
              {allStock.map(item=>{
                ...
                return (
                  <tr key={item.id} className="hover-theme" style={{borderBottom:'1px solid var(--border-light)'}}>
                    ...
                    <td className="py-3 px-4 text-theme-secondary">{item.category||'â€”'}</td>
                    <td className="py-3 px-4 text-theme-secondary">{item.size||'â€”'}</td>
                    <td className="py-3 px-4 font-semibold text-theme-primary">{item.quantity}</td>
                    <td className="py-3 px-4 text-theme-primary">{item.price!=null?`ÂKsh ${Number(item.price).toFixed(2)}`:'â€”'}</td>
                    <td className="py-3 px-4 font-medium text-theme-primary">ÂKsh {val.toFixed(2)}</td>
              ...
              <div key={item.id} className="p-4" style={{borderBottom:'1px solid var(--border-light)'}}>
                <div className="flex items-start justify-between mb-1.5">
                  <div>
                    ...
                    <p className="text-xs text-theme-secondary">{item.category||'â€”'}{item.size?` Â· ${item.size}`:''}</p>
                  </div>
                ...
                <div className="flex gap-4 text-xs text-theme-secondary mt-1">
                  ...
                  <span>Price: <strong className="text-theme-primary">{item.price!=null?`ÂKsh ${Number(item.price).toFixed(2)}`:'â€”'}</strong></span>
                  <span>Value: <strong className="text-theme-primary">ÂKsh {val.toFixed(2)}</strong></span>
                </div>
              </div>
            );
          })}
