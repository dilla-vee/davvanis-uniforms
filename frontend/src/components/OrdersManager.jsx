    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor:'rgba(0,0,0,0.5)' }}
      ...
      <div className={`card rounded-xl shadow-xl w-full ${wide?'max-w-2xl':'max-w-lg'} max-h-[90vh] flex flex-col`} style={{ borderRadius:'0.75rem' }}>
        <div className="flex items-center justify-between p-5 shrink-0" style={{ borderBottom:'1px solid var(--border)' }}>
          ...
          <button onClick={onClose} className="text-xl leading-none text-theme-secondary hover-theme rounded p-1">âœ•</button>
        </div>
    ...
    <div className="space-y-5">
      ...
      <div className="card overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-16 text-theme-secondary"><span className="text-5xl">ðŸ“‹</span><p className="mt-3">No orders yet.</p></div>
        ) : (
          <>
                ...
                <tbody>
                  {orders.map(order=>(
                    <tr key={order.id} className="hover-theme cursor-pointer" style={{borderBottom:'1px solid var(--border-light)'}} onClick={()=>openDetail(order)}>
                      ...
                      <td className="py-3 px-4 font-medium text-theme-primary">{order.client_name||'â€”'}</td>
                      <td className="py-3 px-4 text-xs text-theme-secondary">{order.client_school||'â€”'}</td>
                      ...
                      <td className="py-3 px-4 font-medium text-theme-primary">ÂKsh {(order.total_price||0).toFixed(2)}</td>
                ...
                <div key={order.id} className="p-4 cursor-pointer hover-theme" style={{borderBottom:'1px solid var(--border-light)'}} onClick={()=>openDetail(order)}>
                  <div className="flex items-start justify-between mb-1.5">
                    <div>
                      ...
                      <p className="font-medium text-theme-primary">{order.client_name||'â€”'}</p>
                      <p className="text-xs text-theme-secondary">{order.client_school||'â€”'}</p>
                    </div>
                  ...
                  <div className="flex items-center justify-between mt-2">
                    ...
                    <span className="font-bold text-theme-primary">ÂKsh {(order.total_price||0).toFixed(2)}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div className="p-3 rounded-lg" style={{backgroundColor:'var(--bg-muted)'}}>
                  ...
                  <p className="font-semibold text-theme-primary">{orderDetail.client_name||'â€”'}</p>
                  <p className="text-sm text-theme-secondary">{orderDetail.client_school||'â€”'}</p>
                </div>
                <div className="p-3 rounded-lg" style={{backgroundColor:'var(--bg-muted)'}}>
                    ...
                    <tbody>
                      {(orderDetail.items||[]).map((item,idx)=>(
                        <tr key={idx} style={{borderTop:'1px solid var(--border-light)'}}>
                          ...
                          <td className="py-2 px-3 text-theme-secondary">{item.size||'â€”'}</td>
                          <td className="py-2 px-3 text-theme-primary">{item.quantity}</td>
                          <td className="py-2 px-3 text-theme-primary">ÂKsh {(item.unit_price||0).toFixed(2)}</td>
                          <td className="py-2 px-3 font-medium text-theme-primary">ÂKsh {((item.unit_price||0)*item.quantity).toFixed(2)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <div className="flex justify-end">
                <div className="px-5 py-3 rounded-lg" style={{backgroundColor:'rgba(99,102,241,0.1)'}}>
                  ...
                  <span className="text-lg font-bold" style={{color:'#4f46e5'}}>ÂKsh {(orderDetail.total_price||0).toFixed(2)}</span>
                </div>
              </div>
            </div>
          ...
          <form onSubmit={handleAddSubmit} className="space-y-4">
            ...
            <div><label className="label">Client *</label>
              <select className="input" value={addForm.client_id} onChange={e=>setAddForm(f=>({...f,client_id:e.target.value}))}>
                ...
                {clients.map(c=><option key={c.id} value={c.id}>{c.name} â€” {c.school}</option>)}
              </select>
            </div>
                  ...
                  {addForm.items.map((item,idx)=>(
                    <div key={idx} className="p-3 rounded-lg" style={{border:'1px solid var(--border)',backgroundColor:'var(--bg-muted)'}}>
                      <div className="grid grid-cols-2 gap-2 mb-2">
                        <div><label className="text-xs text-theme-muted mb-1 block">Stock Item</label>
                          <select className="input text-xs" value={item.stock_id} onChange={e=>updateItem(idx,'stock_id',e.target.value)}>
                            ...
                            {stock.map(s=><option key={s.id} value={s.id}>{s.name}{s.size?` (${s.size})`:''} â€” ÂKsh {s.price?.toFixed(2)||'N/A'}</option>)}
                          </select>
                        </div>
                      ...
                      <div className="grid grid-cols-3 gap-2 items-end">
                        ...
                        <div><label className="text-xs text-theme-muted mb-1 block">Price (ÂKsh )</label><input type="number" min="0" step="0.01" className="input text-xs" value={item.unit_price} onChange={e=>updateItem(idx,'unit_price',e.target.value)} /></div>
            ...
            {addForm.items.length>0 && (
              <div className="flex justify-end px-4 py-2 rounded-lg" style={{backgroundColor:'rgba(99,102,241,0.1)'}}>
                ...
                <span className="ml-2 font-bold" style={{color:'#4f46e5'}}>ÂKsh {runningTotal.toFixed(2)}</span>
              </div>
            )}
