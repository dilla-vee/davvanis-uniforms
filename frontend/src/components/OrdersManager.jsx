                ...
                <tbody>
                  {orders.map(order=>(
                    <tr key={order.id} className="hover-theme cursor-pointer" style={{borderBottom:'1px solid var(--border-light)'}} onClick={()=>openDetail(order)}>
                      ...
                      <td className="py-3 px-4 font-medium text-theme-primary">£{(order.total_price||0).toFixed(2)}</td>
              ...
              {orders.map(order=>(
                <div key={order.id} className="p-4 cursor-pointer hover-theme" style={{borderBottom:'1px solid var(--border-light)'}} onClick={()=>openDetail(order)}>
                  ...
                  <div className="flex items-center justify-between mt-2">
                    ...
                    <span className="font-bold text-theme-primary">£{(order.total_price||0).toFixed(2)}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-5">
                    ...
                    <tbody>
                      {(orderDetail.items||[]).map((item,idx)=>(
                        <tr key={idx} style={{borderTop:'1px solid var(--border-light)'}}>
                          ...
                          <td className="py-2 px-3 text-theme-primary">£{(item.unit_price||0).toFixed(2)}</td>
                          <td className="py-2 px-3 font-medium text-theme-primary">£{((item.unit_price||0)*item.quantity).toFixed(2)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <div className="flex justify-end">
                <div className="px-5 py-3 rounded-lg" style={{backgroundColor:'rgba(99,102,241,0.1)'}}>
                  ...
                  <span className="text-lg font-bold" style={{color:'#4f46e5'}}>£{(orderDetail.total_price||0).toFixed(2)}</span>
                </div>
              </div>
            </div>
          ...
          <form onSubmit={handleAddSubmit} className="space-y-4">
                  ...
                  {addForm.items.map((item,idx)=>(
                    <div key={idx} className="p-3 rounded-lg" style={{border:'1px solid var(--border)',backgroundColor:'var(--bg-muted)'}}>
                      ...
                      <div className="grid grid-cols-3 gap-2 items-end">
                        ...
                        <div><label className="text-xs text-theme-muted mb-1 block">Price (£)</label><input type="number" min="0" step="0.01" className="input text-xs" value={item.unit_price} onChange={e=>updateItem(idx,'unit_price',e.target.value)} /></div>
            ...
            {addForm.items.length>0 && (
              <div className="flex justify-end px-4 py-2 rounded-lg" style={{backgroundColor:'rgba(99,102,241,0.1)'}}>
                ...
                <span className="ml-2 font-bold" style={{color:'#4f46e5'}}>£{runningTotal.toFixed(2)}</span>
              </div>
            )}
